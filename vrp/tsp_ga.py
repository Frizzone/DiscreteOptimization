...
#Create random tours
def createTours(customers, vehicle_count):
    customerscopy = customers.copy()
    vehicle_tours = [0 * vehicle_count]
    
    while(len(customerscopy)>0):
        vehicle_tours[random.randint(0, vehicle_count-1)].append(vehicle_tours.pop(random.randint(0, len(customers)-1)))
...
#Selection
#Elitism: select best routes (elit size)
#Fitness proportionate selection: fitness-weighted probability of being selected. (remaning)
